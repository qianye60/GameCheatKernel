...
static uint64_t lil2cpp_base1 = 0;
...
long cooroffest = 0;
  void DrawInit(){
   ...
   /*    
       ...
       */
       libGame_base = getModuleBase( "libGameCore.so");
       lil2cpp_base = getbss("libil2cpp.so:bss");
       
       lil2cpp_base1=getbss("libGameCore.so:bss");
...
draw_list->AddText(text_pos, ImColor(255, 255, 255), "傀儡");
 ...
 if (zhengxing == foeComp) {
    ...
    if (dataTable.heroTemp[i].Id * 100 + 30 == 16930) {
        int value = driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(bingxiang6 + 0x150) + 0x108) + 0xF8) + 0x3C);
        if (value != 0) {
            if (!previousState) {
                previousState = true;
...
float iconHeight = baseIconSize * scaleHeight; // 计算图标高度
    ...
    int 大招最大CD = driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(人物数组 + 0x150) + 0x108) + 0x100) + 0xDC) / 8192000; 
        dataTable.heroTemp[i].Space3 = driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(bingxiang6 + 0x150) + 0x108) + 0x168) + 0xD4) / 8192000;       
          
    dataTable.heroTemp[i].TB4 = driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>
...
//printf("距离  %.0f\n", 距离敌人);
	...
	dataTable.heroTemp[i].TB = driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(bingxiang6+0x150) + 0x150) + 0xC8) + 0x10);
  ...
  // ImGui::GetForegroundDrawList()->AddCircle({(float)CDdrawXY[5][0], CDdrawXY[5][1]}, TXdx, IM_COL32(255, 255, 255, 255), 0, 4.0f);           
    ...
    1.5f); // 线宽
          ...
          opop++;
		   ...
		   if (上帝) {         
		         long sdaddr = ReadValue(ReadValue(lil2cpp_base+0x5DC0)+0xB8)+0x274;
      ...
      if (野怪) {
            ...
            long temp2 = ReadValue(libGame_base + 0x452DE18);
            BuffAddress =
     ...
     if (ESPMenu.是否开启共享) {
        ...
        dataTable.pve[i].X = ReadValue(ReadValue(ReadValue(实体指针+0x2f8)+0x30)+0x10)+0x0;
        dataTable.pve[i].Y = ReadValue(ReadValue(ReadValue(实体指针+0x2f8)+0x30)+0x10)+0x8;
        dataTable.pve[i].hp =
            ReadValue(实体指针 + 0x168) + 0xa8;
        dataTable.pve[i].maxhp =
            ReadValue(实体指针 + 0x168) + 0xb0;

              int 惩戒 = driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(lil2cpp_base+0x2e47b8) + 0xB8) + 0x0) + 0x20) + 0x28) + 0x21c);
				if (召唤师ID == 183) {
                    惩戒伤害 = 惩戒 * 1.25;
                } else {
                    惩戒伤害 = 惩戒;
...
//新libil2cpp.so:bss[1] + 0x4E8980 -> + 0xB8 -> + 0x10 -> + 0x20 -> + 0x20 -> + 0x1E0
    ...
    if (dataTable.pveTemp[i].cd == 0 ||
       ...
       if (兵线) {
            int number; // 兵线数量
            long temp5 = ReadValue(lil2cpp_base1 + 0x165840);
            BxAddress = ReadValue(ReadValue(temp5 + 0x138)+0x108);
            //BxAddress = 适配驱动->read<uint64_t>(适配驱动->read<uint64_t>(temp5 + 0x138) + 0x108);
...
    //眼位（默认开启）
	...
	for (int l = 0; l < 32; l++){
        long 眼位数组 = driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(libGame_base + +0x195968)+ 0x320)+l * 0x18);
     ...
     if (temp == foeComp) {
        ...
        int ywxl = driver->read<int>(driver->read<uint64_t>(眼位数组 + 0x168)+0x98);
            dataTable.YWTemp[cont].coord.X = (float) driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(眼位数组 +0x230 )+ 0x10) + 0x0);
            dataTable.YWTemp[cont].coord.Y = (float) driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(眼位数组 +0x230 ) + 0x10) + 0x8);
