...
void DrawInt() {
    ...
    pid =  driver->initialize((char*)"com.levelinfinite.sgameGlobal"); // get_name_pid((char*)"com.tencent.tmgp.sgame");
...
//0x158918+0x238+0x0 数组遍历0x18*i  +0x168+0x98
    long temp1 = driver->read<uint64_t>(libGame_basebss +  0x158918);
...
if(野怪){
      long temp2 = driver->read<uint64_t>(libGame_basebss + 0x1E18);    //libGameCore.so:bss+0x1E18+0x3B8+0x88+0x120+0x18*i   从0到30   10个英雄  0 10  遍历数组  当前 第二个 第三个  当前数组(自身数组)自身结构体  英雄本身      (i*0x18)    
        BuffAddress =  driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(temp2 + 0x3B8) + 0x88)+0x120) ;
    //     for (int i = 0; i < 21; i++) {
    for (int i = 10; i < 21; i++) {  //国际服
            ...
            //实体坐标  非小地图坐标
            // 228  10  0  和8
             dataTable.pve[i].hp = driver->read<uint64_t>(driver->read<uint64_t>(dataTable.pve[i].cc + 0x3A0) + 0x168) + 0x98;  //血量  实体层 0x168  0x98
            dataTable.pve[i].maxhp = driver->read<uint64_t>(driver->read<uint64_t>(dataTable.pve[i].cc + 0x3A0) + 0x168) + 0xA0;
